...
class App {
    ...
    async handleBuySubmit() {
        ...
        try {
            await storage.addBuyRecord(record);
    ...
    async renderBuyRecords() {
        ...
        let records;
        try {
            records = await storage.getBuyRecords();
        } catch (error) {
    ...
    async deleteSelectedPurchases() {
        ...
        try {
            await storage.deleteBuyRecords(ids);
            await this.renderBuyRecords();
            // 刷新核心指标
    ...
    async updateCalculatorInfo() {
        ...
        try {
            ...
            const stats = await storage.getBuyStats();
            
            // 优先从 history_records 获取最新记录的当前汇率
            let latestRateFromHistory = null;
            try {
                const historyRecords = await storage.getHistoryRecords();
    ...
    async handleCalculate() {
        ...
        let stats;
        try {
            stats = await storage.getBuyStats();
        } catch (error) {
    ...
    async renderHistoryRecords() {
        ...
        let records;
        try {
            records = await storage.getHistoryRecords();
        } catch (error) {
    ...
    async deleteSelectedRecords() {
        ...
        try {
            await storage.deleteHistoryRecords(ids);
            await this.renderHistoryRecords();
            // 刷新核心指标和图表
            if (typeof charts !== 'undefined') {
                await charts.renderCoreMetrics();
